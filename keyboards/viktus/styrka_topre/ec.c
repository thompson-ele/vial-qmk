...
bool ec_matrix_scan(matrix_row_t current_matrix[]) {
    ...
    for (int row = 0; row < sizeof(row_channels); row++) {
        for (int col = 0; col < sizeof(col_pins); col++) {
            uint16_t low_threshold = config.low_threshold;
            uint16_t high_threshold = config.high_threshold;

            // lower threshold for backspace
            if (row == 0 && (col == 15 || col == 16)) {
                low_threshold = 46;
                high_threshold -= 50;
            }

            // Lower threshold for spacebar
            if (row == 4 && (col == 8 || col == 9)) {
                low_threshold -= 5;
                high_threshold -= 10;
            }
            ec_sw_value[col][row] = ec_readkey_raw(col, row);
            updated |= ec_update_key(&current_matrix[row], col, ec_sw_value[col][row], low_threshold, high_threshold);
        }
    }
